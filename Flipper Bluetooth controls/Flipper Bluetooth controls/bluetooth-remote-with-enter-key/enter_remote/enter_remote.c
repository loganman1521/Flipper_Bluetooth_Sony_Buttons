...
static bool enter_remote_read_root_keys(const char* path, GapRootSecurityKeys* keys) {
    uint8_t magic;
    uint8_t version;
    size_t size;
 ...
 */
static void enter_remote_ensure_own_identity(void) {
    GapRootSecurityKeys own_keys;
    bool keys_ok = enter_remote_read_root_keys(HID_KEYS_PATH, &own_keys);

    if(keys_ok) {
        GapRootSecurityKeys default_keys;
        if(enter_remote_read_root_keys(BT_DEFAULT_KEYS_PATH, &default_keys) &&
...
static void enter_remote_bt_start(EnterRemote* app) {
    ...
    enter_remote_ensure_own_identity();
