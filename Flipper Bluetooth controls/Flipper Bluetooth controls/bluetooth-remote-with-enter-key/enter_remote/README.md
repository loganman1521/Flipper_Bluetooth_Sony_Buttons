...
A unique MAC and name are not enough on their own, though. Each keys file also stores the
device's **root security keys** (ERK/IRK) — hosts use the IRK to recognise a device's
*identity* independently of its address. The bt service silently keeps the previous keys in
RAM when an app's keys file doesn't exist yet, so on this app's very first pairing it used
to inherit — and permanently save — the *default* identity's keys. A host paired with both
this app and the stock `Control <name>` remote then saw two bonds with the same identity
keys and kept confusing them (drops, failed re-pairing). Since v1.1 the app writes its own
keys file with freshly generated root keys before starting Bluetooth, and repairs an
existing file that is found to share the default identity, so both pairings coexist.
...
- **Connection drops / re-pairing fails on a machine that is ALSO paired with the stock
  `Control <name>` remote:** this was a real bug in v1.0 — the app's first pairing baked the
  Flipper's default identity keys into its own keys file, so hosts holding both bonds mixed
  them up. Update to v1.1+, then do a one-time clean-up: remove the `Enter …` pairing on the
  host (the `Control <name>` pairing can stay), open the app once (it regenerates its keys
  file automatically), and pair fresh.
